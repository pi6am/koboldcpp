...
static bool common_speculative_are_compatible(
    ...
    if (vocab_type_tgt != vocab_type_dft) {
        SPC_ERR("draft model vocab type must match target model to use speculation but "
...

    {
        ...
        if (vocab_diff > SPEC_VOCAB_MAX_SIZE_DIFFERENCE) {
            SPC_ERR("draft model vocab must closely match target model to use speculation but "
        ...
        for (int i = SPEC_VOCAB_CHECK_START_TOKEN_ID; i < std::min(n_vocab_tgt, n_vocab_dft); ++i) {
            ...
            if (std::strcmp(token_text_tgt, token_text_dft) != 0) {
                SPC_ERR("draft model vocab must match target model to use speculation but "
